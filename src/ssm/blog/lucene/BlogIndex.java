...
public class BlogIndex {
	...
	private IndexWriter getWriter() throws Exception{
		dir = FSDirectory.open(new File("D:\\blog_index"));
		SmartChineseAnalyzer analyzer = new SmartChineseAnalyzer(null);
	...
	public List<Blog> searchBlog(String q) throws Exception{
		...
		QueryParser parser1 = new QueryParser(null,"title", analyzer); //查询标题
		Query query1 = parser1.parse(q);
		
		QueryParser parser2 = new QueryParser(null,"content", analyzer); //查询内容
		Query query2 = parser2.parse(q);
		
		booleanQuery.add(query1, BooleanClause.Occur.SHOULD);
		booleanQuery.add(query2, BooleanClause.Occur.SHOULD);
		
		TopDocs hits = search.search(booleanQuery, 100);
